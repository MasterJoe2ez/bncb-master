...
export class UserSettingComponent implements OnInit {
  ...
  constructor(private _itemtypeService: UserSettingService,
  ...
  load() {
    ...
    this._itemtypeService.requestData().subscribe((snapshot) => {
      ...
      snapshot.forEach((s) => {
        ...
        const _row = new UserSetting(s.val());
  ...
  deleteData(data: UserSetting) {
    this.dialog.open(ConfirmComponent, {
      data: {
        ...
        data_title: 'User',
        data: data.tel + ' : ' + data.address
      }
    }).afterClosed().subscribe((confirm: boolean) => {
      if (confirm) {
        this.snackBar.dismiss();
        this._itemtypeService.removeData(data).then(() => {
          this.snackBar.open('Delete user succeed.', '', {duration: 3000});
          this.addLog('Delete', 'delete User succeed', data, {});
